...
const settingsStateReducer = (state = initialSettings, action) => {
	switch (action.type) {
		...
		case ACTION_TYPES.SET_OS:
			return {
				...state,
				currentWallpaper: action.os === "macos" ? settings.macos_wallpaper : settings.default_wallpaper,
				currentWallpaperId: action.os === "macos" ? "wall_4" : "default_wall"
			};
		default:
			return state;
	}
