...
function Desktop({ isMac }) {
	...
	if (
		...
		return (
			<div className={`screenHeight uk-flex uk-flex-column ${isMac ? 'macos-desktop' : 'windows-desktop'}`}>
				<div className="desktop-container">
					{!isMac && <ActionCenter />}
					{!isMac && <StartMenu />}
					<CalendarOverlay />
					<div className={`app-playground ${isMac ? '' : 'uk-position-right'}`}>
						<div className={`${isMac ? 'macos-icons' : 'uk-position-left uk-flex uk-flex-column'}`}>
